  ...
  return (
    <div className="p-8 h-full overflow-auto bg-muted/5">
      <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{[1,2,3].map(i => <div key={i} className="glass-panel h-48 animate-pulse bg-muted/20"></div>)}</div>
          ...
          <div className="glass-panel p-16 text-center flex flex-col items-center justify-center border border-border/40 shadow-xl">
            <div className="w-20 h-20 rounded-full bg-muted/50 flex items-center justify-center mb-6"><BookmarkIcon className="w-8 h-8 text-muted-foreground" /></div>
            <h3 className="text-xl font-bold text-foreground mb-2">No bookmarks yet</h3>
            <p className="text-muted-foreground max-w-md">Save providers from search results to keep them easy to find and compare.</p>
          </div>
        ) : (
